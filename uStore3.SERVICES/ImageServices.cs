...
namespace uStore3.SERVICES
{
    public class ImageServices
    {
        /// <summary>
        /// Takes an image and makes 2 versions, the larger main version and asmaller thumbnail picture
        /// </summary>
        /// <param name="savePath">Folder path to where we want to save image</param>
        /// <param name="fileName">Base name we want to save image with</param>
        /// <param name="image">Actual file to process, in Image format</param>
        /// <param name="maxImgSize">Largest pixel size desired for either dimension(main version)</param>
        /// <param name="maxThumbSize">Largest pixel size desired for either dimension (thumbnail version)</param>
        ...
        /// <param name="imgName">Name of the image in the folder</param>
        public static void Delete(string path, string imgName)
        {
            ...
            if (imgName != "NoImage.jpg")//make sure not to delete the NoImage image
            {
                if (fullImg.Exists)
                {
                    fullImg.Delete();
